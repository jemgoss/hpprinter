...
package() {
  ...
  cp $srcdir/usr/lib/cups/backend/hp $pkgdir/usr/lib/cups/backend

  install -dm755 $pkgdir/usr/lib/cups/filter
  cp $srcdir/usr/lib/cups/filter/{hpcups,hpps} $pkgdir/usr/lib/cups/filter

  install -dm755 $pkgdir/usr/share/cups/drv/hp
  cp $srcdir/usr/share/cups/drv/hp/hpcups.drv $pkgdir/usr/share/cups/drv/hp

  # We could install all HP ppds if we choose.
  install -dm755 $pkgdir/usr/share/ppd/HP
  cp $srcdir/usr/share/ppd/HP/hp-envy_5660_series.ppd.gz $pkgdir/usr/share/ppd/HP
}
